...
describe('declareVariable', () => {
  describe('declareVariable', () => {
    it('should declare a function if a call expression references one not defined. ', () => {
      ...
      const f = project.createSourceFile('test.ts', `const a = nonDeclared(2,'s')`)
      declareVariable(
        f.getDescendants().find(d => TypeGuards.isIdentifier(d) && d.getText() === 'nonDeclared')! as Identifier,
        project.getTypeChecker(),
        f.getPreEmitDiagnostics()
      )
    ...
    it('should declare a add let to a non declared variable assignation', () => {
      ...
      declareVariable(
        f.getDescendants().find(d => TypeGuards.isIdentifier(d) && d.getText() === 'a')! as Identifier,
        project.getTypeChecker(),
        f.getPreEmitDiagnostics()
      )
      expect(removeWhites(f.getText())).toBe(
