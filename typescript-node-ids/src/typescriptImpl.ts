...
export type TypeScriptId = string

export class TypeScriptImpl implements NodeIds<Node, TypeScriptId> {
  /**
  ...
  install(node: Node): Node {
    let startingId: TypeScriptId | undefined
    ...
    visitChildrenRecursiveDeepFirst(node, (child: Node, index, level) => this.setId(child, `${child.parent ? this.getId(child.parent) : '0'}.${index}`))
    return node
  }
   ...
   */
  setId(node: Node, id: TypeScriptId): void {
   ...
   */
  getIdOrThrow(node: Node): TypeScriptId {
    if (!this.getId(node)) {
   ...
   */
  getId(node: Node): TypeScriptId | undefined {
   ...
   */
  deleteId(node: Node): TypeScriptId | undefined {
   ...
   */
  getNodeById(parent: Node, id: TypeScriptId): Node | undefined {
    ...
    return findChild(parent, child => this.getId(child) === id)
  }
...
export function createTypeScriptImpl(): TypeScriptImpl {
  return new TypeScriptImpl()
}

export const typeScriptImpl = createTypeScriptImpl()