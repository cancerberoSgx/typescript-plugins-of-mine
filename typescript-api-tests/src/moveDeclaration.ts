import Project, { ClassDeclaration, EnumDeclaration, FunctionDeclaration, ImportDeclaration, InterfaceDeclaration, Node, SourceFile, VariableDeclaration, ExportableNodeStructure, ImportDeclarationStructure, ClassDeclarationStructure, Identifier, ImportSpecifier, StringLiteral, ReferenceEntry } from "ts-simple-ast";
  ...
  originalFile.forEachDescendant(d => {
    if (d.getKind() === ts.SyntaxKind.ImportDeclaration) {
      ...
      targetFile.getRelativePathAsModuleSpecifierTo(importDecl.getSourceFile().getDirectory()) + '/' + 
        ((moduleSpecifierSourceFile = importDecl.getModuleSpecifierSourceFile()) && moduleSpecifierSourceFile.getBaseNameWithoutExtension())
...
function importDeclaration2Structure(importDecl: ImportDeclaration): ImportDeclarationStructure {
  let defImport: Identifier | undefined, namedImport: ImportSpecifier[], namespaceImport: Identifier | undefined, moduleSpecifier: StringLiteral
  return {
