...
//   getCompletionsAtPosition: function (fileName, position, options) {
//     const prefix = info.config.prefix || '&%&%'
//     const sourceFile = info.project.getSourceFile(fileName as ts_module.Path)
//     const child = sourceFile.getChildAt(position)

//     info.project.projectService.logger.info(`${PLUGIN_NAME} getCompletionsAtPosition child: ${JSON.stringify(child)} position: ${JSON.stringify(position)} options: ${JSON.stringify(options)}`)

//     const prior = info.languageService.getCompletionsAtPosition(fileName, position, options);
//     prior.entries.push({ name: 'seba', kind: ts.ScriptElementKind..alias, kindModifiers: '', sortText: 'seba' })
